...
class TestStatGrabberHelpers(unittest.TestCase):
    ...
    def test_ensure_date_string_accepts_correct_type(self):
        ...
        self.assertTrue(StatGrabberBase._ensure_date_string('2020-01-01'))
        valid_dt = datetime(year=2020, month=1, day=1)
        self.assertTrue(StatGrabberBase._ensure_date_string(valid_dt))
        self.assertRaises(ValueError, StatGrabberBase._ensure_date_string,
                              '2020-31-01')
        self.assertRaises(ValueError, StatGrabberBase._ensure_date_string,
                              '20200101')
        self.assertRaises(TypeError, StatGrabberBase._ensure_date_string, 1)
    
    def test_ensure_date_string_returns_dt(self):

        from_str = StatGrabberBase._ensure_date_string('2020-01-01')
        from_dt = StatGrabberBase._ensure_date_string(datetime(year=2020, month=1, day=1))
    ...
    def test_convert_datetime_to_unix_returns_int(self):
        ...
        ms_false = StatGrabberBase._convert_datetime_to_unix(self.now, ms=False)
        ms_true = StatGrabberBase._convert_datetime_to_unix(self.now, ms=True)
    ...
    def test_convert_datetime_to_unix_returns_ms_as_default(self):
        ...
        ms_true = StatGrabberBase._convert_datetime_to_unix(self.now)
    ...
    def test_convert_datetime_to_unix_returns_seconds_if_ms_false(self):
        ...
        ms_true = StatGrabberBase._convert_datetime_to_unix(self.now, ms=False)
